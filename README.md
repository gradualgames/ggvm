...
GGVm is a pseudo NES emulator or virtual machine. It was originally
written in an attempt to make it easy for NES homebrew developers
to distribute their games digitally. A couple of games may still be
out there which use GGVm such as Get 'em Gary, but GGVm turned out
to be too cumbersome and limited to be useful. So, I fixed it up
...
instead you can play sound files in your assets directory by
writing to a register in the simulated 6502 cpu.)

Thus ggvm is best for simpler games that are based on NROM
or UnROM mappers which use very few raster effects and have
relatively simple needs for audio playback.
